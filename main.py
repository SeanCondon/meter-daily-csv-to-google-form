...
def extract_import_entries(data: DataFrame, days: int = 7):
    try:
        ...
        last_week_data = last_week_data.drop(columns=['MPRN','Meter Serial Number', 'Read Date and End Time'])
        last_week_data = last_week_data.pivot_table(index='date', values='Read Value', columns=['Read Type'], aggfunc='sum')
